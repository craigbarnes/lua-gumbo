...
# Some distros put the Lua headers in versioned sub-directories, which
# aren't in the default include paths and hence require extra CFLAGS
LUA_CFLAGS    = $(shell $(PKGCONFIG) --cflags $(LUA_PC))

# Some pkg-config files have convenient variables for module paths
# (i.e. were written by people who actually use Lua)
LUA_PC_LMOD   = $(shell $(PKGCONFIG) --variable=INSTALL_LMOD $(LUA_PC))
LUA_PC_CMOD   = $(shell $(PKGCONFIG) --variable=INSTALL_CMOD $(LUA_PC))

# Others were written by idiots, who force us to do this...
